...
- (BOOL) match:(NSString*)path;
{
    ...
    NSUInteger matches = [self.compiledPath numberOfMatchesInString:path options:0 range:NSMakeRange(0, path.length)];
    return matches == 1;
}
...
- (NSDictionary*) parsePathParametersFromPath:(NSString*)path
{
    ...
    NSArray *matches = [self.compiledPath matchesInString:path options:0 range:NSMakeRange(0, path.length)];
